...
The pushy-server provides a feed of events associated with a job, via the
"Server-Sent-Events" protocol (http://www.w3.org/TR/eventsource/).  A client
may open an SSE request to the server, and receive events for the job start,
node votes, node run completion, and job completion.  The events don't provide
any summary information, but there is enough information in the events to
calculate useful statistics about job progress, failure rates, etc.
...
API
---
The SSE feed is restricted by the usual contraints on client requests: the
request must be signed with a valid authentication header, and the associated
user must be in the pushy_jobs_readers group.

When the request is made, a series of past events will be provided, and then
...
Immediately after the job completes, the stream will complete and the server
will close its side of the HTTP connection.

### URL
The feed is available by doing a GET request on:
...
If the header is not included, or the ID is not recognized, then the stream
will start at the beginning.

### Completed jobs
For a brief period after a job completes, the event stream will remain
available.  The request will behave as normal, which means the client will see
the stream of events from the beginning of the job (or if specified, the
Last-Event-ID), through to the job completion, and then the connection will be
immediately closed by the server.  The amount of time before it completes is
configurable, but defaults to 5 seconds.
...
### start
This is issued when the job is requested.  Note that this will only appear
"historically", since the event will be issued by the time a client as access
to the job-id, and the job-id is necessary to request the feed.
...
because an slow or offline node may not respond before the run is complete
...
### quorum_vote
This is issued as each node response to the quorum request.
...
events allows for a more accurate determination of the time required for each
...
------------
### No summary info
As noted above, there is no summary information in the events (e.g. as the
votes are coming in, providing a summary such as "3 success, 2 failure, 5 to
go").  This is because the information is available from the event stream.
The decision was made to avoid providing redundant information in the event
stream, instead requiring the clients to summarize the information as
appropriate.
...
data.  It works adequately, but there are two problems that are impossible to
fix, without either hacking on webmachine, or switching to another framework
such as Cowboy:

### Subscriber close
If the subscriber closes its request before the event stream is complete, the
pushy-server should stop trying to send it data.  But currently the code will
continue to try to send the data.  In the code, the message will reach the
gen_tcp:send(), at which point the data will be dropped.  So there is no
problem in functionality, but the server is doing unnecessary work in this
case.

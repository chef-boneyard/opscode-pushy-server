...
- ZeroMQ Messages:: these are messages from a node telling us about things it's
   doing / has done.
...
This represents the state of a job on a node.  Some states (READY, EXECUTING and
FINISHED) *always* represent the state of the client.  Some states (NEW, VOTING,
TRIGGERED, and ABORTING) are intermediate states for server tracking purposes--
...
|-----------+--------------------------+------------------------------------+----------------------|
| State     | Action on Entering State | Event                              | Next State           |
|-----------+--------------------------+------------------------------------+----------------------|
| VOTING    | Send command message     | Recv NACK                          | FINISHED/unavailable |
| VOTING    |                          | Node State -> ready+this job       | READY                |
| VOTING    |                          | Node State -> other+non-idle       | ABORTING             |
| VOTING    |                          | Job State -> ABORTING/FINISHED     | ABORTING             |
|-----------+--------------------------+------------------------------------+----------------------|
| READY     |                          | Job State -> EXECUTING             | TRIGGERED            |
| READY     |                          | Node State -> non-ready+this job   | ABORTING             |
| READY     |                          | Job State -> ABORTING/FINISHED     | ABORTING             |
|-----------+--------------------------+------------------------------------+----------------------|
| TRIGGERED | Send start message       | Node State -> executing+this job   | EXECUTING            |
| TRIGGERED |                          | Node State -> other+non-ready      | ABORTING             |
| TRIGGERED |                          | Job State -> ABORTING/FINISHED     | ABORTING             |
|-----------+--------------------------+------------------------------------+----------------------|
| EXECUTING |                          | Node State -> idle+executed this   | FINISHED             |
| EXECUTING |                          | Node State -> other                | ABORTING             |
| EXECUTING |                          | Job State -> ABORTING/FINISHED     | ABORTING             |
|-----------+--------------------------+------------------------------------+----------------------|
| ABORTING  | Send abort message       | Node State -> idle+last job=this   | FINISHED             |
|-----------+--------------------------+------------------------------------+----------------------|
| FINISHED  |                          |                                    |                      |
|-----------+--------------------------+------------------------------------+----------------------|
...
|-------------+----------------------------------+----------------------|
| FINISHED    | All Nodes Finished               | FINISHED             |
|-------------+----------------------------------+----------------------|
...
The Job state machine keeps a tally of which jobs are in which states, so that
it can calculate some tallies and take care of quorum conditions:

- Quorum Success:: if we are voting, and "ready" job nodes > X%.
- Quorum Failure:: if we are voting, and finished/down nodes > Y% or there are no job nodes.
- All Nodes Finished:: if all job nodes are finished.

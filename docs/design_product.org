...
|-----------+------------------------------------+--------------------------|
| READY     | Job State -> EXECUTING             | Send start message       |
| READY     | Node State -> up                   | Resend if job executing  |
| READY     | Node Report -> ready+this job      | Resend if job executing  |
| READY     | Node Report -> executing+this job  | ->EXECUTING              |
| READY     | Node Report -> other               | ->ABORTING               |
...
The Job state machine keeps a tally of which nodes are running the job and

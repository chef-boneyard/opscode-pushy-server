...
*** Overall communications structure
    There are a number of ways to implement the communication structure. 


**** Client listens
   The client chooses some port to bind a PULL to, and then advertises it to the server, perhaps via the
   heartbeat channel. The command server issues commands to this port, and possibly includes a port
   to report status back via PUSH connection. Otherwise this could be contained in the configuration
   data.

   Advantages:
   This enables having a separate socket on the server for each job, which would ease the process of
   distributing the work of managing the job, since a separate erlang process could be used for each
   job. Having an ephemeral socket also allows us to tear down the socket and destroy any messages
   waiting for a dead node.

   Disadvantages: This will require the server to setup/teardown many sockets as it individually
   connects to each client and sends the command. This seems somewhat against the ZeroMQ
   style. Having the server connect to the client means that the client cannot live behind any kind
   of NAT device without added work, a limitation we haven't yet had to insert into our design.
...
**** Server listens
   ...
   Advantages:
   This reduces the number of ZeroMQ connections we have to maintain. It preserves the pattern of the
   client always connecting to the server.

   Disadvantages:
   This makes all the jobs go through the same socket, and will require some mux/demux work to route
   messages where the need to go. (This is a relatively straightforward pattern for ZeroMQ though.)


**** Handoff model
   As in the 'Server listens' model, clients connect to the command server via a ROUTER/DEALER
   pattern. When we initiate a job, the command sent to the clients includes a port for further
   communication. 

   Advantages:
   This minimizes the long lived sockets in the system, and allows a separate socket/process to
   handle the connection, moving some of the traffic off of the main command connection, while
   preserving the client connects model. 

   Disadvantages:
   This is a more complex protocol. The actual amount of traffic it moves to the separate connection
   isn't huge; the main server still needs to contact every node in the job to tell it where to
   listen (The protocol would only have small number of messages going back and forth for each node,
   so it would only alter the traffic by a small constant factor).
